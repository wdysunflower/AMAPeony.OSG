package com.peony.osg.view.widget.pull;
...
import com.peony.osg.view.widget.pull.util.PtrLocalDisplay;
 ...
 * refreshing / hide header view while refreshing It defines
 * {@link com.peony.osg.view.widget.pull.PtrUIHandler}, which allows you customize the UI easily.
 */
public class PtrFrameLayout extends ViewGroup {
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (arr != null) {
            ...
            mRatioOfHeaderHeightToRefresh =
                    arr.getFloat(R.styleable.PtrFrameLayout_ptr_ratio_of_header_height_to_refresh,
                            mRatioOfHeaderHeightToRefresh);
            ...
            mPullToRefresh =
                    arr.getBoolean(R.styleable.PtrFrameLayout_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        CLog.d(String.format("onMeasure frame: width: %s, height: %s, padding: %s %s %s %s",
        ...
        if (mHeaderView != null) {
            ...
            CLog.d(String
                    .format("onMeasure header: height: %s, topMargin: %s, bottomMargin: %s, headerHeight: %s",
                            mHeaderView.getMeasuredHeight(), lp.leftMargin, lp.rightMargin,
                            mHeaderHeight));
        }
        ...
        if (mContent != null) {
            ...
            CLog.d(String.format("onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
                    getMeasuredWidth(), getMeasuredHeight(), lp.leftMargin, lp.topMargin,
                    lp.rightMargin, lp.bottomMargin));
            CLog.d(String.format("onMeasure, mCurrentPos: %s, mLastPos: %s, top: %s", mCurrentPos,
                    mLastPos, mContent.getTop()));
        }
    }
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            CLog.d(String.format("onLayout header: %s %s %s %s", left, top, right, bottom));
        }
        if (mContent != null) {
            MarginLayoutParams lp = (MarginLayoutParams) mContent.getLayoutParams();
            final int left = paddingLeft + lp.leftMargin;
            ...
            CLog.d(String.format("onLayout content: %s %s %s %s", left, top, right, bottom));
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (mCurrentPos > POS_START) {
                    ...
                    CLog.d(String.format("call onRelease when user release"));
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mDisableWhenHorizontalMove
                        ...
                        && (Math.abs(offsetX) > mPagingTouchSlop || Math.abs(offsetX) > 3 * Math
                                .abs(offsetY))) {
                    if (frameIsNotMoved()) {
                ...
                CLog.v(String
                        .format("ACTION_MOVE: offsetY:%s, mCurrentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s",
                                offsetY, mCurrentPos, moveUp, canMoveUp, moveDown, canMoveDown));
    ...
    private void movePos(float deltaY) {
        ...
        if ((deltaY < 0 && mCurrentPos == POS_START)) {
            CLog.e(String.format("has reached the top"));
            return;
        }
    ...
    private void updatePos() {
        ...
        if (mLastPos == POS_START && mCurrentPos != POS_START && mPtrUIHandlerHolder.hasHandler()) {
            if (mStatus == PTR_STATUS_INIT) {
                ...
                CLog.i(String.format("PtrUIHandler: onUIRefreshPrepare, mAutoScrollRefreshTag %s",
    ...
    public void setRefreshCompleteHook(PtrUIHandlerHook hook) {
        ...
        hook.setResumeAction(new Runnable() {
            @Override
            public void run() {
                CLog.d("mRefreshCompleteHook resume.");
    ...
    private void performRefresh() {
        ...
        if (mPtrUIHandlerHolder.hasHandler()) {
            ...
            CLog.i("PtrUIHandler: onUIRefreshBegin");
        }
        if (mPtrHandler != null) {
    ...
    protected void onPtrScrollAbort() {
        if (mCurrentPos > 0 && mAutoScrollRefreshTag > 0) {
            CLog.d("call onRelease after scroll abort");
            onRelease(true);
        }
    }
    ...
    protected void onPtrScrollFinish() {
        if (mCurrentPos > 0 && mAutoScrollRefreshTag > 0) {
            CLog.d("call onRelease after scroll finish");
            onRelease(true);
        }
    }
    ...
    final public void refreshComplete() {
        ...
        if (delay <= 0) {
            CLog.d("performRefreshComplete at once");
            ...
            CLog.d(String.format("performRefreshComplete after delay: %s", delay));
        }
    }
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (mCurrentPos != POS_START && !ignoreHook && mRefreshCompleteHook != null) {
            CLog.d("notifyUIRefreshComplete mRefreshCompleteHook run.");

            mRefreshCompleteHook.takeOver();
            return;
        }
        if (mPtrUIHandlerHolder.hasHandler()) {
            CLog.i("PtrUIHandler: onUIRefreshComplete");
    ...
    private void sendCancelEvent() {
        CLog.d("send cancel event");
        MotionEvent e =
                MotionEvent.obtain(mDownEvent.getDownTime(), mDownEvent.getEventTime()
                        + ViewConfiguration.getLongPressTimeout(), MotionEvent.ACTION_CANCEL,
                        mDownEvent.getX(), mDownEvent.getY(), mDownEvent.getMetaState());
        dispatchTouchEventSupper(e);
    }

    private void sendDownEvent() {
        CLog.d("send down event");
        final MotionEvent last = mLastMoveEvent;
        MotionEvent e =
                MotionEvent.obtain(last.getDownTime(), last.getEventTime(),
    ...
    class ScrollChecker implements Runnable {
        ...
        public void run() {
            ...
            if (deltaY != 0) {
                CLog.v(String
                        .format("scroll: %s, start: %s, to: %s, mCurrentPos: %s, current :%s, last: %s, delta: %s",
        ...
        private void finish() {
            CLog.v(String.format("finish, mCurrentPos:%s", mCurrentPos));
            reset();
            onPtrScrollFinish();
        }
        ...
        public void tryToScrollTo(int to, int duration) {
            ...
            CLog.d(String.format("tryToScrollTo: start: %s, distance:%s, to:%s", mStart, distance,
                    to));
            removeCallbacks(this);
